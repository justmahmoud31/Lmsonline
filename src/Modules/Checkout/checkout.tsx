      ...
      {courseData && (
        <div className="bg-white p-6 rounded-xl shadow space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            <Info label="الفصل الدراسي" value={courseData.term === "FIRST" ? "الأول" : "الثاني"} />
            <Info label="عدد الأجزاء" value={courseData._count.Part.toString()} />
            <Info label="عدد الدروس" value={courseData._count.Lesson.toString()} />
            <Info label="عدد الامتحانات" value={courseData._count.Exam.toString()} />
            <Info label="السعر" value={`${courseData.price} د.ك`} />
          </div>
          <div className="flex">
            <button className="bg-main text-white p-1 w-full rounded-md cursor-pointer">اشتراك</button>
          </div>
        </div>
      )}
    </div>
